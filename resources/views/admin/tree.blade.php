...
@section('content')
    @if ($user->parent_id)
        <p><a class="btn btn-primary" href="{{route('admin.Tree',$user->parent_id)}}">Дерево Лидера</a></p>
    @else
        <p><a href="{{route('admin.MainTree',$user['id'])}}" class="btn btn-primary">Деревья</a></p>
    @endif
    ...
    <div style="overflow: auto;padding-bottom: 50px">
        <div class="children">
            @php $main = 0; @endphp
            @component('admin.treeUser',['user'=>$user,'maxColumnCount'=>5,'i'=>0,'j'=>0,'row'=>0,'main'=>$main,'tree'=>$tree])
